...
//Fixed error when user selects 'use threshold' if no threshold has been set
...
public class Correlation_Analysis_Sasha implements PlugIn 
	{
...
public boolean showDialog() 
{
	...
	if(useThreshold)
		{
		if((ipthresh2.getMinThreshold()!=ipthresh2.NO_THRESHOLD)&&(ipthresh1.getMinThreshold()!=ipthresh1.NO_THRESHOLD) )
			{useThreshold=false;
			IJ.showMessage("No threshold detected. Inactivating 'Use Threshold'");
			}
		}
			...
			for (int x=0; x<=rwidth; x++) 
				...
				if (mask!=0)
					...
					if(((ch1>=ch1threshmin)&&(ch2>=ch2threshmin))||!useThreshold)
						...
						scaledZ2= 256-(int)((double)256*(((double)ch2-(double)ch2min)/((double)ch2max-(double)ch2min)));
						
						count = plot.getPixel(scaledZ1+45, scaledZ2);
						count++;
				                	plot.putPixel(scaledZ1+45, scaledZ2, count);	
						if (count<65535) count++;
						plot16bit.putPixel(scaledZ1, scaledZ2, count);
						}
					}
				}
			}
