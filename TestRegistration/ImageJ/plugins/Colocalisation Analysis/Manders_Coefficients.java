...
ImageStack redStack, greenStack, blueStack;
...
public boolean showDialog() {
	...
	gd.addChoice("Ch1", titles, titles[0]);

	gd.addChoice("Ch2", titles, titles[1]);
...
//generate stacks for processing
	img1 = imp1.getStack();
//	if (imp1.getType()!=imp1.GRAY8&&imp1.getType()!=imp1.GRAY16&&imp2.getType()!=imp2.GRAY16 &&imp2.getType()!=imp1.GRAY8) 
	if (imp1.getType()!=imp1.GRAY8&&imp1.getType()!=imp1.GRAY16) 
			{
			redStack = new ImageStack(width,height);
     			greenStack = new ImageStack(width,height);
			blueStack =new ImageStack(width,height);
			//ip1 = img1.getProcessor(i);
			//split colour processors
		
			for (int cs=1; cs<=imp1.getStackSize(); cs++)
				{
				cp = (ColorProcessor)img1.getProcessor(cs);
...
public void correlate(ImageStack img1, ImageStack img2) 
	...
	String fileName = Ch1fileName;
	if (imp1.getType()==imp1.GRAY8||imp1.getType()==imp1.GRAY16) 	
		fileName=fileName+Ch2fileName;
	else 
	...
	if ((imp1.getType()==imp1.GRAY8)||(imp1.getType()==imp1.COLOR_256)|| (imp1.getType()==imp1.COLOR_RGB)) bitDepth=255;
	...
	if(threshold)
		{
		if(ip2.getMinThreshold()!=ImageProcessor.NO_THRESHOLD)
			{ch2threshmin=(int)ip2.getMinThreshold();
			ch2threshmax=(int)ip2.getMaxThreshold();
			}
		if(ip1.getMinThreshold()!=ImageProcessor.NO_THRESHOLD)
			{
...
//	new ImagePlus("red "+i , ip1).show();
//	new ImagePlus("green", ip2).show();
...
//finished pixel counting
	...
	if (showFreqPlot) 
		...
		imp3.setTitle(fileName + " Freq. CP " + dualChannels[dualChannelIndex]);
	...
	String Headings = "Image\tChannels\tPearson's\tManders' R\tch1:ch2\tM1\tM2\tCh1 Thresh\tCh2 Thresh\n";
	if ((!headingsSet))
		{ 
...
//	      	IJ.write(Headings);
	...
	IJ.run("Enhance Contrast", "saturated=90 normalize equalize normalize_all use");
	imp4.setTitle(fileName + " CP "  + dualChannels[dualChannelIndex]);
	}
